<?php 
    ...
    && isset($_POST['contactNumber']) && isset($_POST['email']) && isset($_POST['password']) && isset($_POST['password2'])) {
        ...
        elseif($password != $password2){
            ...
            $sql = "INSERT INTO tbl_userinfo (firstname, middlename, lastname, user_status) VALUES ('$firstname', '$middlename', '$lastname', '1')";
            ...
            if($conn->query($sql) === TRUE) {
                ...
                if($conn->query($sql) === TRUE) {
                    $sql = "INSERT INTO tbl_usertype (user_id, user_type) VALUES ('$user_id', 'admin')";
                    
                    if($conn->query($sql) === TRUE) {
                        $sql = "INSERT INTO tbl_contactinfo (user_id, address, phoneNum) VALUES ('$user_id', '$address', '$contactNumber')";

                        if($conn->query($sql) === TRUE) {
                            header("Location: adminlist.php?msg= Admin account added successfully");
                            exit();
                        }
                    }
                }
